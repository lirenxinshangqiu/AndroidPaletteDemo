...
object PaletteUtils {
    ...
    fun getPicturePixel(bitmap: Bitmap): ArrayList<Int> {
        val width = bitmap.width
        val height = bitmap.height
        // 保存所有的像素的数组，图片宽×高
        val pixels = IntArray(width * height)
        bitmap.getPixels(pixels, 0, width, 0, 0, width, height)
        val rgb: ArrayList<Int> = ArrayList()
...
//        val picturePixel = getPicturePixel(small(bitmap))
        val picturePixel = getPicturePixel(bitmap)
        //计数相同颜色数量并保存
        val color2: HashMap<Int, Int?> = HashMap()
